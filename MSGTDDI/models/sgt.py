...
class SGTEncoder(FairseqEncoder):
    ...
    def forward(self, batched_data, perturb=None, masked_tokens=None,interstate=False, return_attention=False, **unused):
        # print(batched_data['x'].shape)
        inner_states, graph_rep,att = self.graph_encoder(
            batched_data,
            perturb=perturb,
            return_attention=return_attention or interstate,
        )
        if interstate:
...
class SGTDDIWrapper(BaseFairseqModel):
    ...
    def forward(self, batched_data, return_representation=False, return_attention=False, return_explain=False):
        # 拆分出两个图数据
        ...
        # 分别得到两个图的表示
        explain_mode = return_attention or return_explain
        if explain_mode:
            out1 = self.base_model(data1, return_attention=True)
            out2 = self.base_model(data2, return_attention=True)
            z1 = out1["output"]
            z2 = out2["output"]
        else:
            out1 = out2 = None
        ...
        if explain_mode:
            return {
                ...
                "att2": out2["attention"],
                "meta": {
                    "idx": batched_data.get("idx"),
                    "global_idx1": batched_data.get("global_idx1"),
                    "global_idx2": batched_data.get("global_idx2"),
                    "cur_id1": batched_data.get("cur_id1"),
                    "cur_id2": batched_data.get("cur_id2"),
                    "num_atoms1": batched_data.get("num_atoms1"),
                    "num_atoms2": batched_data.get("num_atoms2"),
                    "num_subtokens1": batched_data.get("num_subtokens1"),
                    "num_subtokens2": batched_data.get("num_subtokens2"),
                    "identifiers1": batched_data.get("identifiers1"),
                    "identifiers2": batched_data.get("identifiers2"),
                },
            }
        return logits
