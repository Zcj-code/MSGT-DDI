...
#             "targets": targets[:logits.size(0)].detach().cpu(),
#         }
...
#         # 收集所有预测值和真实值
#         preds = torch.cat([log["preds"] for log in logging_outputs], dim=0)
#         targets = torch.cat([log["targets"] for log in logging_outputs], dim=0)
#
#         # ✅ 计算指标
#         mae = torch.nn.functional.l1_loss(preds, targets, reduction="mean").item()
#         mse = torch.nn.functional.mse_loss(preds, targets, reduction="mean").item()
#         rmse = mse ** 0.5
#
#         metrics.log_scalar("mae", mae, sample_size, round=6)
#         metrics.log_scalar("rmse", rmse, sample_size, round=6)
#
#     @staticmethod
