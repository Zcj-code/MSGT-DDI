...
def eval(args, use_pretrained, checkpoint_path=None, logger=None):
    ...
    with torch.no_grad():
        ...
        for i, sample in enumerate(progress):
            ...
            if args.criterion=='multilabel_multiclass_cross_entropy':
                y = model(**sample["net_input"]).squeeze(0).argmax(1)
    ...
    if use_pretrained:
        ...
        if args.metric == "auc":
            ...
            mae = np.mean(np.abs(y_true - y_pred).numpy())
...
def main():
    ...
    if args.pretrained_model_name != "none":
        ...
        # for checkpoint_fname in ['checkpoint_best.pt','checkpoint_last.pt']:
        for checkpoint_fname in os.listdir(args.save_dir):
            ...
            print('-----------------------------------------------------------------result: ',res)
            res_lis.append(res)
            print(np.max(res_lis),np.argmax(res_lis),np.min(res_lis),np.argmin(res_lis))
