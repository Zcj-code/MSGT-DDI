...
def floyd_warshall(adjacency_matrix):
    ...
    cdef np.ndarray[long, ndim=2, mode='c'] M = adj_mat_copy
    cdef np.ndarray[long, ndim=2, mode='c'] path = -1 * np.ones([n, n], dtype=np.int64)
...
def gen_edge_input(max_dist, path, edge_feat):
    ...
    edge_feat_copy = edge_feat.astype(np.float32, order='C', casting='safe', copy=True)
    assert path_copy.flags['C_CONTIGUOUS']
    assert edge_feat_copy.flags['C_CONTIGUOUS']

    cdef np.ndarray[np.float32_t, ndim=4, mode='c'] edge_fea_all = -1. * np.ones(
        [n, n, max_dist_copy, edge_feat.shape[-1]], dtype=np.float32)
