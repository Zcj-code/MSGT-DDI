...
from .collator import collator, collatorcontrust,collator_adj_pair, collator_adj_target_pair
...
class BatchedDataDataset_DDISubstructure(FairseqDataset):
    def __init__(
        self, dataset, max_node=128, multi_hop_max_dist=5, spatial_pos_max=1024
    ):
        super().__init__()
    ...
    def __getitem__(self, index):
        item = self.dataset[int(index)]
        item._original_index = index  # 添加原始索引标记
        return item
...
class TargetDataset(FairseqDataset):
    def __init__(self, dataset, node_level_task=False, max_node=128):
        super().__init__()
        self.dataset = dataset
        self.node_level_task = node_level_task
        self.max_node = max_node  # 新增过滤条件
    ...
    def collater(self, samples):
        return collator_adj_target_pair(
            samples,
            max_node=self.max_node,
            node_level_task=self.node_level_task
        )
