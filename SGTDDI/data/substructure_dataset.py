...
from .wrapper import graph_data_modification_pair, encode_token_pair_tensor, \
    encode_token_pair_tensor_with_adj, preprocess_item_pair
        ...
        if isinstance(idx, int):
            ...
            if self.transform is not None:
                ...
                if (not self.use_transform_cache) or self.only_substructure:
                    ...
                    def get_subtensor(idx, env, graph_key):
                        """
                        从 LMDB 中读取与 `graph_key` 相关的邻居数据
                        :param idx: 数据的索引
                        :param env: LMDB 环境
                        :param graph_key: 要获取的键 ('graph1_neighbors' 或 'graph2_neighbors')
                        :return: 获取的邻居数据
                        """
                        if env is None:
                            return None
                        ...
                        if isinstance(idx, torch.Tensor):
                            idx = idx.item()  # 提取标量值
                        raw = txn.get(str(idx).encode())  # 使用 idx 读取数据
                        if raw is None:
                            raise ValueError(f"Substructure tensor for {idx} not prepared")

                        # 将 raw 数据转换为字典并根据 graph_key 获取对应的数据
                        data = eval(raw)
                        if graph_key not in data:
                            raise ValueError(f"Key {graph_key} not found in the data for idx {idx}")

                        return data[graph_key]

                    # 使用相同的 idx 读取两个分子图的邻居数据
                    sub1 = self.transform.substructure_as_tensor(
                        get_subtensor(str(self.recomputeind.compute(idx)), self.lmdb_tensor_env, 'graph1_substructure'),
                        get_subtensor(str(self.recomputeind.compute(idx)), self.lmdb_neighbor_env, 'graph1_neighbors')
                    )

                    # 读取第二个图的邻居数据
                    sub2 = self.transform.substructure_as_tensor(
                        get_subtensor(str(self.recomputeind.compute(idx)), self.lmdb_tensor_env, 'graph2_substructure'),
                        get_subtensor(str(self.recomputeind.compute(idx)), self.lmdb_neighbor_env, 'graph2_neighbors')
                    )
                    if self.only_substructure:
                        ...
                        for i in range(self.transform_cache_number):
                            t1, a1 = self.transform(item, sub1,graph_index=1)
                            t2, a2 = self.transform(item, sub2,graph_index=2)
                            item.subgraph_tensor_res.append((t1.tolist(), t2.tolist()))
                            item.sorted_adj_res.append((a1.tolist(), a2.tolist()))
                        item.y = item.y.reshape(-1)
                        return item
                    else:
                        subgraph1, adj1 = self.transform(item, sub1,graph_index=1)
                        subgraph2, adj2 = self.transform(item, sub2,graph_index=2)
                else:
                    # 读取 transform 缓存
                    txn = self.transform_cache_env.begin(write=False)
                    id = np.random.randint(self.transform_cache_number)
                    raw = txn.get((str(self.recomputeind.compute(idx))+ f"_{id}").encode())
                    if raw is None:
                        raise ValueError("Transform cache not prepared.")
                    (subgraph1, adj1), (subgraph2, adj2) = eval(raw)
                    subgraph1, subgraph2 = torch.tensor(subgraph1), torch.tensor(subgraph2)
                    adj1, adj2 = torch.tensor(adj1), torch.tensor(adj2)

                    # 修改图对数据结构
                item = graph_data_modification_pair(item, subgraph1, subgraph2,adj1,  adj2)
                ...
                if self.extra_method == 'token':
                    ...
                    item = encode_token_pair_tensor(item, self.args.atom_id_max, self.args.edge_id_max)
                elif self.extra_method == 'adj':

                    item = encode_token_pair_tensor_with_adj(item, self.local_attention_on_substructures)


                ############################
                # === Attention 编码等 === #
                ############################
            if (not self.not_re_define) or (not hasattr(item, 'attn_bias1')):

                item.idx = idx
                item.y = item.y.reshape(-1)
                item = preprocess_item_pair(
                    item,
                    local_attention_on_substructures=self.local_attention_on_substructures,
                    continuous_feature=self.continuous_feature
                )
