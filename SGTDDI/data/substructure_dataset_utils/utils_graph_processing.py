...
def automorphism_orbits(edge_list, print_msgs=False, only_graph=False,**kwargs):
...
def subgraph_isomorphism_vertex_counts(edge_index, **kwargs):
    ...
    edge_list=list(edge_index.transpose(1,0).cpu().numpy())
    edge_list=[[int(edge_list[i][0]),int(edge_list[i][1])] for i in range(len(edge_list))]
...
def subgraph_isomorphism_vertex_extraction(edge_index,input_graph=None, **kwargs):
    ##### vertex structural identifiers #####
    ...
    subgraphs = set()
    for sub_iso_curr in sub_iso:
        tem = list(sub_iso_curr.a)
        tem.sort()
        subgraphs.add(tuple(tem))
    return subgraphs
...
def subgraph_2token(count_fn, data, subgraph_dicts, subgraph_params,graph_index):
    #### Remove self loops and then assign the structural identifiers by computing subgraph isomorphisms ####

    # 根据 graph_index 选择对应的 edge_index 和 edge_features
    if graph_index == 1:
        edge_index = data.edge_index1
        edge_features = data.edge_attr1 if hasattr(data, 'edge_attr1') else None
        x=data.x1
    elif graph_index == 2:
        edge_index = data.edge_index2
        edge_features = data.edge_attr2 if hasattr(data, 'edge_attr2') else None
        x=data.x2
    else:
    ...
    if edge_features is not None:
        ...
        setattr(data, f'edge_attr{graph_index}', edge_features)  # 更新对应图的边特征
    else:
    ...
    G_gt = gt.Graph(directed=subgraph_params['directed'])
    edge_list = list(edge_index.transpose(1, 0).cpu().numpy())
    edge_list = [[int(edge_list[i][0]), int(edge_list[i][1])] for i in range(len(edge_list))]
    G_gt.add_edge_list(edge_list)
    # gt.stats.remove_self_loops(G_gt)
    ...
    for subgraph_dict in subgraph_dicts:
        kwargs = {'subgraph_dict': subgraph_dict,
                  ...
                  'num_nodes': num_nodes,
                  'directed': subgraph_params['directed']}
        subgraphs = count_fn(edge_index, input_graph=G_gt, **kwargs)
        identifiers.append(subgraphs)
    # setattr(data, 'edge_index', edge_index)
    # setattr(data, 'identifiers', identifiers)
